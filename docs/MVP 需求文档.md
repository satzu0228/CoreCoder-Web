...
Agent Runtime    Session（内存态状态机）
   │
   ├─ Web workspace 路径约束: read_file / grep / glob
   └─ 改造原生工具（不新增工具类，直接在原实现上加确认流程）:
        ├─ edit_file / write_file  先算 diff → 确认 → 通过才 write
        └─ bash                    危险命令从"直接拒绝"改为"询问用户"
```
...
**最终实现（2026-08-08）**：采用 `web/_confirmable.py` 的 `request_confirmation(action, payload)` 函数，而不是修改 Tool 继承体系。`edit_file`、`write_file`、危险 `bash` 共用该入口；入口用锁串行展示并行确认请求，以匹配 MVP 只有一个确认框的前端模型。CLI 没有 SSE emitter 时不进入等待流程。
...
- **并行确认按顺序处理。** 普通工具仍由 `as_completed()` 完成一个就发送一个 `tool_end`；多个确认型工具同时出现时，`_confirmation_gate` 保证前端一次只看到一个确认框。后续请求会等前一个批准、拒绝或超时后再展示。
...
MVP 只做**内存态**，不接 Redis/DB。验证目标不是"重启服务后能恢复"，而是"**浏览器刷新页面后，如果有一个确认还没处理，UI 能重新显示出确认框**"——`GET /api/session/pending` 查询当前 pending confirmation 即可。
...
│   ├── session.py         GET  /api/session/pending
...
│                            confirm_required（EditFileTool/WriteFileTool/BashTool）最终都走这一个函数
└── confirm_registry.py     ConfirmRegistry：event_id ↔ threading.Event 的加锁封装，
                             管 create/wait/resolve，超时与 /api/confirm 到达后都会自动清理
```

`workspace_fs.py` 是 Web 文件工具（`read_file`/`write_file`/`edit_file`/`glob`/`grep`）和文件树 API 共用的路径安全逻辑，CLI 不受该边界影响。`events.py` 和 `confirm_registry.py` 是确认机制的基础模块，`EditFileTool`/`WriteFileTool`/`BashTool` 的确认逻辑与 `agent.chat()` 的 `tool_end` 都建立在其上。
...
SSE 事件：
  ...
  tool_end          {"id","result"}
  confirm_required   {"id","action":"edit_file"|"write_file"|"bash","file_path"?,"diff"?,"command"?,"reason"?}
  done               {"tokens"?}
...
**不做独立 Diff 页面**：`DiffViewer.vue` 是独立组件，由确认框中的“View Full Diff”按钮打开 `DiffViewerModal`，不把 Monaco 逻辑写死在 `ConfirmModal` 或 `ToolCallCard` 内。`ToolCallCard` 只展示工具状态和摘要，`DiffViewer` 专门封装 Monaco 的加载和渲染。
...
| M3   | `EditFileTool` 在写入前生成 diff 并确认；`ConfirmModal` 弹窗，`DiffViewer.vue` 由独立 modal 展示 Monaco diff | 确认闭环、组件职责分离 |
| M4   | 危险 `BashTool` 与 `WriteFileTool` 接入通用 `request_confirmation()`；`GET /api/session/pending` 支持刷新恢复；并行确认顺序展示 | 人机协作闭环、CLI 兼容、基本状态恢复 |
...
> 基于开源 minimal coding agent（CoreCoder）二次开发，在 `edit_file`/`write_file`/危险 `bash` 中引入 Human-in-the-loop 审批：以事件总线打通 Tool 与 SSE，通过加锁的 `ConfirmRegistry` 和串行确认门管理阻塞等待、超时清理与并发展示，并提炼通用 `request_confirmation()`；同时保留 CLI 行为、约束 Web 文件工具的 workspace 路径，前端用 Vue3 + Monaco 展示执行过程与代码 diff。
